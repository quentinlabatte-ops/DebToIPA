...
class DebToIPA {
    ...
    static func convert(_ url: URL, statusUpdate: (String) -> ()) throws -> SavedIpa {
        ...
        // Extract .deb
        let extractedDir = try extractDeb(url, to: tempDir.appendingPathComponent("deb_extracted"), statusUpdate: statusUpdate)
        let applicationsDir = extractedDir.appendingPathComponent("Applications/")
        guard fm.fileExists(atPath: applicationsDir.path) else { throw ConversionError.unsupportedApp }
        let appURLs = try fm.contentsOfDirectory(at: applicationsDir, includingPropertiesForKeys: nil)
...
//                }
        ...
        let tempIpaURL = zipFilePath.deletingPathExtension().appendingPathExtension("ipa")
        try? fm.removeItem(at: tempIpaURL)
        try fm.moveItem(at: zipFilePath, to: tempIpaURL)

        // Move out of tmp so it survives cleanup() (called at the top of every convert())
        // and app relaunches, instead of silently disappearing from the saved list.
        let destIpaURL = try persistIpa(at: tempIpaURL)
    ...
    /// Directory for IPAs that should survive `cleanup()` and app relaunches, unlike tmp.
    private static var savedIpasDir: URL {
        let dir = fm.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("SavedIPAs", isDirectory: true)
    ...
    /// Moves a freshly created .ipa from tmp into `savedIpasDir`, uniquifying the name on collision.
    private static func persistIpa(at url: URL) throws -> URL {
        let base = url.deletingPathExtension().lastPathComponent
        var destURL = savedIpasDir.appendingPathComponent(url.lastPathComponent)
        var counter = 1
        while fm.fileExists(atPath: destURL.path) {
            destURL = savedIpasDir.appendingPathComponent("\(base)-\(counter).ipa")
            counter += 1
        }
        ...
        for (header, dataInts) in reader {
            ...
            for entry in tarContainer {
                if entry.info.type == .directory {
                    ...
                    // Path-based API stores entry.info.linkName verbatim (POSIX symlink semantics),
                    // so relative targets stay relative to the link itself. The URL-based API instead
                    // resolved relative targets against the process's working directory, breaking them.
                    try fm.createSymbolicLink(atPath: extractedDir.appendingPathComponent(entry.info.name).path, withDestinationPath: entry.info.linkName)
                } else {
